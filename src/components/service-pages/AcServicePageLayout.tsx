...
export type AcPageVisualVariant = "repair" | "installation" | "maintenance" | "hub";
...
interface AcServicePageLayoutProps {
  visualVariant: AcPageVisualVariant;
  helmetTitle: string;
...
const bgSrc = (n: "1" | "2" | "3") => `/background-${n}.png`;
...
function MidSection({ variant }: { variant: AcPageVisualVariant }) {
  if (variant === "repair") {
    return (
      ...
      <section className="py-12 px-4 bg-[#001c70]">
        <div className="container max-w-6xl mx-auto grid md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            />
            <p className="text-white/80 text-sm mt-3 text-center md:text-left">Cooling equipment</p>
          </div>
          <div>
            <img
              src="/van2.png"
              ...
              className="w-full rounded-2xl border-4 border-white/20 shadow-elevated object-cover max-h-[280px]"
            />
            <p className="text-white/80 text-sm mt-3 text-center md:text-left">Install crew &amp; fleet</p>
          </div>
        </div>
      </section>
    );
        ...
        <div className="container max-w-5xl mx-auto flex flex-col lg:flex-row gap-10 items-center">
          ...
          <ul className="text-white space-y-4 lg:flex-1">
            <li className="flex gap-3">
              ...
              <span className="text-white/90">Coils, refrigerant, and electrical checks each visit</span>
            </li>
            <li className="flex gap-3">
              <span className="text-primary font-bold">✓</span>
              <span className="text-white/90">Filter and airflow review for Utah dust and dry air</span>
            </li>
            <li className="flex gap-3">
              <span className="text-primary font-bold">✓</span>
              <span className="text-white/90">Spring tune-ups before peak summer demand</span>
            </li>
          </ul>
        </div>
      </section>
...
const AcServicePageLayout = ({
  ...
  featuresHeading,
  featuresIntro,
}: AcServicePageLayoutProps) => {
  const contentBg: "1" | "2" | "3" =
    visualVariant === "installation" ? "2" : visualVariant === "maintenance" ? "3" : "1";
  const contentMax = visualVariant === "installation" || visualVariant === "hub" ? "max-w-5xl" : "max-w-4xl";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <CurvedDivider
        topColor={light}
        bottomColor={dark}
        accentColor={visualVariant === "hub" ? skyBlue : mint}
        direction="down"
        transparentTop
        className="relative z-10"
      />
        ...
        <div className="container max-w-2xl mx-auto relative z-10 text-center">
          <div className="rounded-3xl border border-border bg-card shadow-card px-8 py-10 md:px-10 md:py-12">
            ...
            <p className="text-muted-foreground mb-8">
              Call Peak HVAC Services or use our contact form for AC repair, installation, or maintenance in Utah.
            </p>
...
export default AcServicePageLayout;
